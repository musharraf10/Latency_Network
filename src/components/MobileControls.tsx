...
const MobileControls = () => {
  ...
  const { realTimeEnabled, showHeatmap, setShowHeatmap, filters, setFilters } = useStore();
          ...
          >
            <div className="p-4">
              ...
              <div className="flex items-center justify-between mb-6">
                <h2 className={`font-bold text-lg ${isDark ? "text-white" : "text-slate-900"}`}>Controls</h2>
              ...
              {/* Tab Navigation */}
              <div className={`flex gap-1 mb-4 rounded-lg p-1 transition-colors ${
              ...
              <AnimatePresence mode="wait">
                ...
                >
                    ...
                    <Card className={`transition-colors ${
                      ...
                      <CardContent className="p-4 space-y-3">
                        <div className="flex justify-between items-center">
                          <span className={`text-sm ${isDark ? "text-slate-300" : "text-slate-600"}`}>Status</span>
                        ...
                        <div className="flex justify-between items-center">
                          <span className={`text-sm ${isDark ? "text-slate-300" : "text-slate-600"}`}>Connections</span>
                          <span className="text-green-400 font-mono text-sm font-medium">
                            {statistics.activeConnections}/{statistics.totalConnections}
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className={`text-sm ${isDark ? "text-slate-300" : "text-slate-600"}`}>Real-time</span>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="space-y-2">
                          {["binance", "coinbase", "kraken", "wazirx"].map((exchangeId) => {
                            const exchangeNames = {
                              binance: "Binance",
                              coinbase: "Coinbase",
                              kraken: "Kraken",
                            ...
                            <div key={exchangeId} className="flex items-center justify-between">
                              <span className={`text-sm ${isDark ? "text-slate-300" : "text-slate-600"}`}>
                                {exchangeNames[exchangeId as keyof typeof exchangeNames]}
                              </span>
                              <input 
                                type="checkbox" 
                                className="rounded" 
                                checked={filters.exchanges.length === 0 || filters.exchanges.includes(exchangeId)}
                                onChange={() => handleExchangeToggle(exchangeId)}
                              />
                            </div>
                          )})}
                        </div>
                          ...
                          {["AWS", "GCP", "Azure"].map((provider) => (
                            <div key={provider} className="flex items-center justify-between">
                              ...
                              <input 
                                type="checkbox" 
                                className="rounded" 
                                checked={filters.cloudProviders.includes(provider as "AWS" | "GCP" | "Azure")}
                                onChange={() => handleProviderToggle(provider as "AWS" | "GCP" | "Azure")}
                              />
                            </div>
                          ))}
                        </div>
                  ...
                  {activeTab === "settings" && (
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <span className={`text-sm ${isDark ? "text-slate-300" : "text-slate-600"}`}>Heatmap</span>
                        <input
                          type="checkbox"
                          checked={showHeatmap}
                      ...
                      <div className="flex justify-between items-center">
                        <span className={`text-sm ${isDark ? "text-slate-300" : "text-slate-600"}`}>Auto-rotate</span>
                        <input type="checkbox" className="rounded" defaultChecked />
                      </div>
                      <div className="flex justify-between items-center">
                        <span className={`text-sm ${isDark ? "text-slate-300" : "text-slate-600"}`}>Animations</span>
                        <input type="checkbox" className="rounded" defaultChecked />
                      </div>
        ...
        >
          <div className={`flex justify-between items-center text-xs font-medium ${
            ...
            <div className="text-center">
              ...
              <div className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}>Avg</div>
            </div>
            <div className="text-center">
              <div className="font-bold text-sm">{statistics.activeConnections}</div>
              <div className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}>Active</div>
            </div>
            <div className="text-center">
              <div className="font-bold text-sm">{statistics.avgPacketLoss}%</div>
              <div className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}>Loss</div>
            </div>
            <div className="text-center">
